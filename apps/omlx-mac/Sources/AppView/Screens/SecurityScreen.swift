...
struct SecurityScreen: View {
    ...
    var body: some View {
        ...
        .task { loadStatusKitPrefs() }
    }

    private func loadStatusKitPrefs() {
...
///
/// When enabled, oMLX will automatically add its bundle ID to the StatusKit
/// approval list on launch (the same logic MenubarVisibilityWatcher uses for
/// the silent auto-fix). This requires Full Disk Access permission.
private struct StatusKitSection: View {
    ...
    var body: some View {
        SectionHeader(
            String(localized: "security.section.statuskit",
                   defaultValue: "StatusKit (macOS Tahoe)",
                   comment: "Section header for the StatusKit approval toggle"),
            subtitle: String(localized: "security.section.statuskit.sub",
                             defaultValue: "Automatically approve oMLX in System Settings so the menubar icon appears.",
                             comment: "Subtitle for the StatusKit section")
        )

        ListGroup {
            Row(
                label: String(localized: "security.statuskit.toggle",
                              defaultValue: "Auto-Approve in StatusKit",
                              comment: "Row label for the StatusKit auto-approve toggle"),
                sublabel: String(localized: "security.statuskit.toggle.sub",
                                 defaultValue: "Requires Full Disk Access. Disabled by default.",
                                 comment: "Sublabel for the StatusKit toggle"),
                isLast: true
            ) {
                Toggle("", isOn: $enabled)
                    .labelsHidden()
