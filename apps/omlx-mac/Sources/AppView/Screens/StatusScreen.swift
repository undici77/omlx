...
/// Inline consent notice shown when the user has not yet granted explicit
    ...
    var body: some View {
        HStack(spacing: 8) {
            Image(systemName: "info.circle.fill")
                .font(.system(size: 13))
                .foregroundStyle(theme.textSecondary)
            Text(String(localized: "status.updates.consent_notice",
                        defaultValue: "Update checks contact GitHub to look for new releases. No personal data is sent. Enable above to opt in.",
                        comment: "Inline consent notice explaining what update checks do"))
                .font(.omlxText(11))
                .foregroundStyle(theme.textSecondary)
                .lineLimit(3)
            Spacer()
            Button(String(localized: "status.updates.consent_grant",
                          defaultValue: "I understand",
                          comment: "Button to grant consent for update checks")) {
...
private struct UpdatesSection: View {
    ...
    var body: some View {
        ListGroup {
            ...
            if !updates.consentGiven {
                ConsentNoticeRow(updates: updates)
            }
            Row(
