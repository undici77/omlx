...
final class MenubarVisibilityWatcher {
    ...
    /// Whether StatusKit auto-fix is enabled by the user in Security screen.
    private var statuskitAutoFixEnabled: Bool {
    ...
    private func runAutofixFlow() {
        ...
        guard statuskitAutoFixEnabled else { return }
        let result = MenubarIconRecovery.fixStatusKitPermission()
        MenubarLog.write(
            "auto-fix result: success=\(result.success) fda=\(result.needsFullDiskAccess) — \(MenubarIconRecovery.oneLine(result.message))"
        )
        if result.needsFullDiskAccess {
            MenubarIconRecovery.showStatusKitAccessDeniedAlert()
            return
        }
        MenubarIconRecovery.showAutofixResultAlert(success: result.success, message: result.message)
    }
}
