    ...
    def _reap_remote_ranks(self, *, runner: SSHRunner = subprocess.run) -> None:
        ...
        for rank, host in enumerate(self.deployment.hosts):
            ...
            script = (
                ...
                "try:\n"
                "  wide_env=dict(os.environ, COLUMNS='2000')\n"
                "  res=subprocess.run(['ps','-p',str(pid),'-o','args='],capture_output=True,text=True,check=False,env=wide_env)\n"
                "  cmd=res.stdout.strip()\n"
                "  if not cmd:\n"
                "    res=subprocess.run(['ps','-p',str(pid),'-o','command='],capture_output=True,text=True,check=False,env=wide_env)\n"
                "    cmd=res.stdout.strip()\n"
                "except Exception:\n"
                "  cmd=''\n"
