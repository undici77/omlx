...
def _register_module() -> None:
    ...
    if _MODULE_NAME in sys.modules:
        current = sys.modules[_MODULE_NAME]
        if getattr(current, "__file__", "") == str(file_path):
            return
...
def apply_spark2_5_patch() -> bool:
    ...
    try:
        ...
        if getattr(module, "__file__", "") == "mock_mlx.py" or not hasattr(module, "Model"):
            raise ModuleNotFoundError(name=_MODULE_NAME)
