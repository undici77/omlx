...
# Detect if we're running under the mlx_mock (Linux/CI). Audio/VLM integration
# tests that patch mlx_vlm.utils functions need the real MLX library.
try:
    from omlx.utils.mlx_mock import MockMLXFinder
    _HAS_MOCK = any(isinstance(f, MockMLXFinder) for f in __import__("sys").meta_path)
except Exception:
    _HAS_MOCK = False

def _has_real_mlx():
    """Check if real MLX is available (not under mock)."""
    return HAS_MLX and not _HAS_MOCK
...
class TestPrepareVisionInputs:
    ...
    @pytest.mark.skipif(lambda: not _has_real_mlx(), reason="Real MLX required for audio tests")
    @patch("mlx_vlm.utils.prepare_inputs")
    @patch("mlx_vlm.prompt_utils.apply_chat_template")
    ...
    @patch("mlx_vlm.utils.prepare_inputs")
    @patch("mlx_vlm.prompt_utils.apply_chat_template")
    def test_audio_empty_list_not_passed_via_real_mlx(self, mock_vlm_act, mock_prepare):
        """Empty audio list is equivalent to None — real MLX path."""
        engine = self._setup_engine_for_vision(model_type="gemma4")
...
class TestFormatMessagesForVLMTemplate:
    ...
    @pytest.mark.skipif(lambda: not _has_real_mlx(), reason="Real MLX required for audio+image formatting")
